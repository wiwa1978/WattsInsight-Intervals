...
interface CreditsConsumedDataPoint {
  period: string;
  consumed: number;
}

interface CreditsConsumedChartProps {
  dailyData: CreditsConsumedDataPoint[];
  weeklyData: CreditsConsumedDataPoint[];
  monthlyData: CreditsConsumedDataPoint[];
  yearlyData: CreditsConsumedDataPoint[];
}
...
const fillDailyData = (data: CreditsConsumedDataPoint[]): CreditsConsumedDataPoint[] => {
  ...
  const result: CreditsConsumedDataPoint[] = [];
...
const fillWeeklyData = (data: CreditsConsumedDataPoint[]): CreditsConsumedDataPoint[] => {
...
const fillMonthlyData = (data: CreditsConsumedDataPoint[]): CreditsConsumedDataPoint[] => {
  // Normalize the keys from backend data
  const dataMap = new Map(data.map(d => [normalizeDate(d.period), d]));
  const result: CreditsConsumedDataPoint[] = [];
  const today = new Date();
...
const fillYearlyData = (data: CreditsConsumedDataPoint[]): CreditsConsumedDataPoint[] => {
  // Normalize the keys from backend data
  const dataMap = new Map(data.map(d => [normalizeDate(d.period), d]));
  const result: CreditsConsumedDataPoint[] = [];
...
export function CreditsConsumedChart({
  ...
  const t = useTranslations("admin.dashboard.creditsConsumedChart");
